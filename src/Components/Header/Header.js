    ...
    return(
        <div className={"header-container " + (isDarkMode ? "dark" : "light")}>
            <div className="header-icon">
                <FontAwesomeIcon icon={faQuestionCircle} size={Math.min(window.innerHeight, window.innerWidth)<315 ? "" : "xl"} className={props.gameMessageState ? "" : "interactive-icon"} onClick={()=>{if(buttonActive && !props.gameMessageState){
                                                                                                                                                                                                                props.informationState[1](true);
                                                                                                                                                                                                            }}}/>
            </div>
            <div className="header-icon" style={{color: isDarkMode ? "#121213" : "white"}}>
                <FontAwesomeIcon icon={faQuestionCircle} size={Math.min(window.innerHeight, window.innerWidth)<315 ? "" : "xl"} />
            </div>
            <div className="title">
                <h1>Wordle</h1>
            </div>
            <div className="header-icon">
                <FontAwesomeIcon icon={faMoon} size={Math.min(window.innerHeight, window.innerWidth)<315 ? "" : "xl"} className={props.gameMessageState ? "" : "interactive-icon"} 
                                                                                            onClick={()=>{
